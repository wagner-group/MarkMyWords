...
def prepare_for_hull(d, bounds, baselines, direction=None):
    ...
    if type(d) == dict:
        ...
        values = np.clip(np.array([list(v) for v in d.values()]), 0, max(bounds))
        keys = list(d.keys())
    else:
    ...
    # Normalize baselines
    baselines = [[b/bounds[i] for b in baselines[i]] for i in range(len(bounds))]
    ...
    if len(direction) == 2:
        ...
        values = np.vstack([values, [min_quality, max_watermarking]])
        ...
        max_quality, max_watermarking, max_robustness = baselines[0,1], baselines[1,1], baselines[2,1]
        min_quality, min_watermarking, min_robustness = baselines[0,0], baselines[1,0], baselines[2,0]
        # 0 quality, perfect watermarking, perfect robustness
        values = np.vstack([values, [min_quality, max_watermarking, max_robustness]])
        # 0 quality, perfect watermarking, 0 robustness
        values = np.vstack([values, [min_quality, max_watermarking, min_robustness]])
        # 0 quality, 0 watermarking, perfect robustness
        values = np.vstack([values, [min_quality, min_watermarking, max_robustness]])
        # Ideal quality, no watermarking, no robustness
        values = np.vstack([values, [max_quality, min_watermarking, min_robustness]])
        # Ideal quality, no watermarking, perfect robustness
        values = np.vstack([values, [max_quality, min_watermarking, max_robustness]])
        # 0 everywhere
        values = np.vstack([values, [min_quality, min_watermarking, min_robustness]])
...
def hull_volume(hull, points, bounds, baseline_quality, direction=None):
    ...
    for face in hull.faces:
        ...
        volume += sum([np.dot(up, v) for v in values[face]]) * surface_area * orientation / len(bounds)


    # Update points
    hull = replace(hull, points={k: values[i] for i,k in enumerate(hull.keys)}, volume=volume)
...
def summarize_robustness(stats, threshold = 0.8, existing_hull=None):
    ...
    for model in stats:
        ...
        metric_a = {attack: (stats[model][attack].quality, stats[model][attack].percent_watermarked) for attack in stats[model] if attack is not None and 'dipper' not in attack and "gpt" not in attack}
        metric_b = {attack: (stats[model][attack].quality, stats[model][attack].efficiency) for attack in stats[model] if attack is not None and 'dipper' not in attack and "gpt" not in attack}

        threshold_metric_a = {k: metric_a[k] for k in metric_a if metric_a[k][0] >= threshold*base_q}
        threshold_metric_b = {k: metric_b[k] for k in metric_b if metric_b[k][0] >= threshold*base_q}

        try:
            best_attack_a, score_a = sorted(threshold_metric_a.items(), key=lambda x: (x[1][1], -x[1][0]))[0]
            score_a = score_a[1]

            best_attack_b, score_b = sorted(threshold_metric_b.items(), key=lambda x: (-x[1][1], -x[1][0]))[0]
            score_b = score_b[1]
        ...
        if existing_hull is None:
            hull = convex_hull(metric_a, [base_q,1], ((0,base_q), (base_w,0)), direction=[1,1])
        else:
            points = [metric_a[k] for k in existing_hull[model].hull.keys if k not in existing_hull[model].hull.added]
            hull = hull_volume(existing_hull[model].hull, points, [base_q,1], ((0,base_q), (base_w,0)), direction=[1,1])

        hull = replace(hull, volume=max(0,(base_w) - hull.volume))

        summary[model] = Robustness(hull=hull, efficiency_threshold=score_b, percentage_threshold=score_a, best_attack_efficiency=best_attack_b, best_attack_percentage=best_attack_a)
...
def find_convex_hull(data, baselines, max_seq_len, ignore_robustness=False):
    ...
    for (model,temp), points in data.items():
        local_baselines = [(0, baselines[temp]), (max_seq_len, 0), (0,0.5)] if not ignore_robustness else [(0, baselines[temp]), (max_seq_len, 0)]
        rtn[(model,temp)] = convex_hull(points, [1,max_seq_len,0.5] if not ignore_robustness else [1,max_seq_len], local_baselines, direction=[1,-1,1] if not ignore_robustness else [1,-1])
...
def convex_hull_validation(orig_hull, data, baselines, max_seq_len, ignore_robustness = False):
    rtn = {}
    for (model,temp), points in data.items():
        ...
        all_points = [(0,0) if ignore_robustness else (0,0,0) for k in hull.keys if k not in hull.added]
        ...
        local_baselines = [(0, baselines[temp]), (max_seq_len, 0), (0,0.5)] if not ignore_robustness else [(0, baselines[temp]), (max_seq_len, 0)]
        hull = hull_volume(hull, all_points, [1,max_seq_len,0.5] if not ignore_robustness else [1,max_seq_len], local_baselines, direction=[1,-1,1] if not ignore_robustness else [1,-1])
        rtn[(model,temp)] = hull
...
def find_threshold(data, thresholds, baselines):
    ...
    for (model,temp), points in data.items():
        ...
        if thresholds[1] >= 0:
            q_t, r_t = q_baseline*(1-float(thresholds[0])), r_baseline*(1-float(thresholds[1]))
            best_label, best_values = min(list(points.items()), key=lambda x: 32000*(x[1][0] < q_t) + 32000*(x[1][2] < r_t) + x[1][1])
        else:
            q_t = q_baseline*(1-float(thresholds[0]))
            best_label, best_values = max(list(points.items()), key=lambda x: -32000*(x[1][0] < q_t) + x[1][2])
