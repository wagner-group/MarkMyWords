...
def init_attacks(config, dispatch_queue=None, results_queue=None, synonym_cache=None, names_only=False):
    ...
    for name, params in ParaphraseAttack.get_param_list(reduced=not config.paraphrase):
        attack_list[name] = ParaphraseAttack(*params, queue=dispatch_queue, resp_queue=results_queue)  if not names_only else True
    for name, params in SynonymAttack.get_param_list():
        attack_list[name] = SynonymAttack(*params, generation_queue=dispatch_queue, resp_queue=results_queue, cache=synonym_cache) if not names_only else True
...
def perturb_process(task_queue, writer_queue, results_queue, dispatch, config, synonym_cache):
    ...
    while True:
        ...
        if attack_name is None:
            ...
            generation = replace(generation, response=attack.warp(generation.response, generation.prompt), attack=attack_name)
            writer_queue.put(generation)
...
def run(config_file, generations=None):
    ...
    for generation in generations:
        wid, gid = (generation.watermark.to_dict(True, True) if generation.watermark is not None else generation.temp), generation.id
    ...
    dipper_queue, translate_queue = global_manager.Queue(), global_manager.Queue()
    if config.paraphrase:
        for i in range(config.dipper_processes):
            processes.append(multiprocessing.Process(target=dipper_server, args=(dipper_queue,'cuda:{}'.format(i))))
            processes[-1].start()

    for d in config.get_devices():
        process_count = int(math.floor(torch.cuda.get_device_properties(d).total_memory / 2000000000)) if d != "cpu" else 1
        for _ in range(process_count):
            processes.append(multiprocessing.Process(target=translate_process, args=(translate_queue, ['en','fr','ru'], d)))
            processes[-1].start()
            time.sleep(5)
    ...
    if config.paraphrase:
        for i in range(config.openai_processes):
            processes.append(multiprocessing.Process(target=openai_process, args=(openai_queue, config.openai_key, openai_cache)))
            processes[-1].start()

    # Setup dispatch process
    dispatch_queues = {'dipper': dipper_queue, 'translate': translate_queue, 'openai': openai_queue}
    ...
    for _ in range(config.threads):
        ...
        processes.append(multiprocessing.Process(target=perturb_process, args=(task_queue, writer_queue, results_queues[-1], dispatch_queues, config, synonym_cache)))
        processes[-1].start()
    ...
    # Setup writer
    writer = multiprocessing.Process(target=writer_process, args=(writer_queue, config, task_count))
    writer.start()
