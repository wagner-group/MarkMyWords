...
from watermark_benchmark.utils.classes import BenchmarkResults, Generation, WatermarkSpec, AggregateResults, Threshold
from watermark_benchmark.utils.summarize import summarize_robustness, verify_threshold, find_threshold
...
def run(config_file, generations=None):
    ...
    config.hull_axis = [tuple(v) if type(v) == list else v for v in config.hull_axis]
    config.aggregate_thresholds = [tuple(v) if type(v) == list else v for v in config.aggregate_thresholds]
    ...
    if not config.load_from_save:
        # Load generations`
        generations = Generation.from_file(get_output_file(config)) if generations is None else generations

        attack_dict = {str(g.watermark): {} for g in generations if g.watermark is not None}
        perturbed_list = set()
        non_watermarked = {}
        ...
        summary_attack = {k: {a: BenchmarkResults(  quality = np.mean([g.rating for g in gs if (k, g.id) in perturbed_list]),\
                                            efficiency = np.median([g.efficiency for g in gs if (k, g.id) in perturbed_list]),\
                                            percent_watermarked = np.mean([1 if g.efficiency != math.inf else 0 for g in gs if (k, g.id) in perturbed_list])) for a,gs in attack_dict[k].items()} for k in attack_dict}

        summary = {k: BenchmarkResults(  quality = np.mean([g.rating for g in d[None]]),\
                                    efficiency = np.median([g.efficiency for g in d[None]]),\
                                    percent_watermarked = np.mean([1 if g.efficiency != math.inf else 0 for g in d[None]]),\
                                    robustness = None) for k,d in attack_dict.items()}
        ...
        if config.validation:
            ...
            baselines = {float(temp): np.mean([g.rating for g in gs]) for temp, gs in non_watermarked.items()}
    ...
    if config.validation:
        ...
        robustness_summary = summarize_robustness(summary_attack, threshold = config.threshold, existing_hull=robustness_hull)
    else:
        robustness_summary = summarize_robustness(summary_attack, threshold = config.threshold)
    ...
    summary = {k: replace(summary[k], robustness = robustness_summary[k]) for k in summary}
    ...
    for axis in config.hull_axis:
        ...
        for k in summary:
            ...
            if axis == "rng" or (type(axis) != str and any("rng" == v for v in axis)):
                if ws['rng'] == "Internal" and ws['hash_len'] == 0 or ws['rng'] != "Internal" and ws['key_len'] == 1:
                    ws['rng'] = "None" 
            ...
            if key not in agg:
                agg[key] = {}
            agg[key][k] = tuple([round(v,3) for v in (summary[k].quality, summary[k].efficiency, summary[k].robustness.hull.volume)])
        if not len(agg.keys()):
            continue
        ...
        if config.validation:
            for thsd in config.aggregate_thresholds:
                thresholds[axis][thsd] = verify_threshold(agg, thresholds[axis][thsd])
        else:
            for thsd in config.aggregate_thresholds:
    ...
    aggregate_results = [AggregateResults(axis, value, temp, Threshold(thsd), q, e, r) for axis in thresholds for thsd in thresholds[axis] for (value, temp), (_, (q, e, r)) in thresholds[axis][thsd].items()]
        ...
        for axis in thresholds:
            for thsd in thresholds[axis]:
                for key, val in thresholds[axis][thsd].items():
                    outfile.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(axis, key, *thsd, val[0], *val[1]))
