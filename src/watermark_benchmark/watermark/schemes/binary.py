...
from watermark_benchmark.watermark.templates.verifier import EmpiricalVerifier, Verifier
...
class BinaryGenerator(Watermark):
    ...
    def process(self, logits, previous_tokens, ids):
        ...
        for bit_index in range(self.binarizer.L):
            ...
            prob_done = probs[:,representations[:, bit_index] == -1].sum(axis=1) / prob_sum
            branch_filter[prob_done>0.5]=False
            if not branch_filter.sum():
                break

            # Compute probability and get randomness
            p = probs[:,representations[:, bit_index] == 1].sum(axis=1) / prob_sum
            ...
            criteria = representations.expand(N,*representations.shape)[:,:,bit_index] != choice[:,bit_index].expand(self.binarizer.V,N).t()
            probs[criteria] = 0
...
class BinaryVerifier(Verifier):
    ...
    def verify(self, tokens, index=0, exact=False):
        ...
        if len(tokens.shape) == 0:
            return [(False, 0, 0, 0)]
        binary_tokens = self.binarizer.to_bit(tokens.to(self.rng.device)).squeeze()
        ...
        for i in range(tokens.shape[-1]):
            ...
            xi.append([self.rng.rand_index(seed, i).item() for i in range(bitlen)] + [-1 for _ in range(max_bitlen-bitlen)])

        xi = torch.Tensor(xi).to(self.rng.device)

        v = -(xi*binary_tokens + (1-xi)*(1-binary_tokens)).abs().log() * mask
...
class BinaryEmpiricalVerifier(EmpiricalVerifier):
    ...
    def score_matrix(self, tokens, random_values, index=0):
        ...
        indices = torch.vstack((((torch.arange(KL, device=self.rng.device).reshape(-1,1).repeat(1,SL).flatten())), torch.arange(SL, device=self.rng.device).repeat(KL)%SL)).t().reshape(KL, -1, 2).to(self.rng.device)
        ...
        for r in range(1 + (max_bitlen // 100)):
            ...
            xi_local = random_values[0,:,range_low:range_high].reshape(-1,range_high-range_low)

            # Binary tokens has shape SL x L, xi has shape KL x L. We only want to sum coordinates that are not -1.
            v = ((xi_local[indices[:,:,0]] * binary_tokens_local[indices[:,:,1]] + (1 - binary_tokens_local[indices[:,:,1]]) * (1-xi_local[indices[:,:,0]])).abs()).log() * mask_local[indices[:,:,1]]
            if rslt is None:
                rslt = v.sum(axis=-1)
            else:
                rslt += v.sum(axis=-1)
    ...
    def random_score_matrix(self, tokens, random_shape, shared_randomness, binary_tokens=None, index=0):
        ...
        indices = torch.vstack((((torch.arange(KL, device=self.rng.device).reshape(-1,1).repeat(1,SL).flatten())), torch.arange(SL, device=self.rng.device).repeat(KL)%SL)).t().reshape(KL, -1, 2)
        xi = torch.cuda.FloatTensor(L,max_bitlen, device=self.rng.device).uniform_(0,1)[shared_randomness,:]
        ...
        for r in range(1 + (max_bitlen // 100)):
            ...
            binary_tokens_local = binary_tokens[:, range_low:range_high]
            mask_local = mask[:, range_low:range_high]
            xi_local = xi[:,range_low:range_high].reshape(-1,range_high-range_low)
            # Binary tokens has shape SL x L, xi has shape KL x L. We only want to sum coordinates that are not -1.
    ...
    def pre_compute_baseline(self, max_len=1200, runs=250):
        ...
        binary_tokens = torch.randint(0,2, (max_len*self.binarizer.avg_bit_length,)).to(self.rng.device).reshape(max_len, self.binarizer.avg_bit_length)
        ...
        for run in range(runs):
            scores = self.random_score_matrix(None, (1, L, self.binarizer.avg_bit_length), shared_randomness, binary_tokens)
